...
to setup-intelligents
  ...
  ask origins [
    hatch-cars 1 [
      ...
      set heading 90
      ;set route set-route origin destination
      set moving? false
      set reached? false
      set previous origin
      set speed 0.05
...
to go
  if ticks = 0 [
    set acts user-input "acts?"
  ]

  
  if count cars with [reached?] = count cars [stop]
  ...
  ask cars with [not moving? and not reached? and intelligent?][
    set direction item 0 acts
    set acts remove-item 0 acts
    if direction = "R" [set heading 90]
    if direction = "U" [set heading 0]
    if direction = "D" [set heading 180]
  ...
  ;ask roads with [traffic > 0][set color red]
