...
to initialize
  clear-all
  file-close-all
  set q matrix:make-constant (( network-size ) ^ 2 ) 2 0
  set counts matrix:make-constant (( network-size ) ^ 2 ) 2 0
  set alpha 0.1
  set beta 0.99
  set halt? false
  set intel-time 0
...
to load
  ...
  set rewards [] 
  set states []
  set states lput [origin] of one-of cars with [intelligent?] states
...
to-report act [state]
  ...
  ;print row
  if [x] of state != (network-size - 1) and [y] of state != 0 [
    let mx max row
...
to update [sts trj rews]
  ...
  repeat rpt [
    ...
    set sts but-last sts
    let action last trj
    set trj but-last trj
    let rew last rews
    set rews but-last rews 
    
    if action = "D" [ set action 0]
    if action = "R" [ set action 1]
    ;if action = "D" [ set action 2]
    ...
    if nxtnum = ((network-size ^ 2) - 1) [
      matrix:set-row q nxtnum n-values 2 [lastrew]
    ]
...
to trajectory
  ask cars with [intelligent?][
    ...
    while [current != destination] [
      let direct act current
      set trj lput direct trj
...
to do-go
  load
  trajectory
  let trj [route] of one-of cars with [intelligent?]
  let rews n-values (( network-size - 1 ) * 2) [0]
  repeat 20 [
    load
    ask one-of cars with [intelligent?] [set route trj]
    reset-ticks 
    while [ not halt? ] [go]
    ;print [rewards] of one-of cars with [intelligent?]
    set rews ( map + [rewards] of one-of cars with [intelligent?] rews)
  ]
  let final-rews []
  foreach rews [set final-rews lput (? / 20) final-rews]
...
to go
  if count cars with [reached?] = count cars or ticks = 4000 [
    if ticks = 4000 [set intel-time 4000]
    set halt? true
    stop
  ]
...
to get-time
  ...
  repeat 20 [
    ...
    while [ not halt? ] [opt-go]
     set times lput intel-time times
     set rews lput intel-rew rews
  ]
  wait 2
  ;print times
  print mean(times)
