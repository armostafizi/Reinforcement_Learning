...
to setup-agents
  ...
  ;ask one-of intersections with [ xcor = min-pxcor - 0.5 + (world-width ) / (road-network-size + 1) ][
  ask one-of intersections with [ xcor < -15][   
    hatch-cars 100 [
      ...
      set heading 90
      set orig myself
      ;set dest one-of intersections with [ abs(xcor - ( max-pxcor + 0.5 - (world-width) / (road-network-size + 1))) < 0.001]
...
to go
  if count cars with [color = green] = count cars [ stop ]
  ask cars [
    ...
    if not reached? [
      ...
      [
        ...
        ask road [who] of orig next [
          set traffic traffic + 1
        ]
        set route remove-item 0 route
        set heading towards intersection next
        set moving? True
        set orig intersection next
      ]
    
      if distance intersection next < 0.1 [
        set moving? False
        if intersection next = dest [
          set reached? True
          set color Green
        ]
      ]
